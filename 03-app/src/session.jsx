...
export function SessionProvider({ children }) {
  ...
  async function loadProfile(schoolId) {
    const profile = await client.api.request('/me');
    setUser(profile); setSchool(profile.memberships.find(item=>item.school.id===schoolId) || profile.memberships[0] || null); setReady(true);
  }
  ...
  return <Context.Provider value={{ ...client, user, school, setSchool, signedIn: Boolean(user && ready), signIn, unlock, signOut, reloadProfile:loadProfile }}>{children}</Context.Provider>;
}
